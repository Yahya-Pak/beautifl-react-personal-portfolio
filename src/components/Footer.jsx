import { ArrowUp } from 'lucide-react'
import React from 'react'

const Footer = () => {
  return (
    <footer className='py-6 px-4 bg-card relative border-t border-border mt-12 pt-8 flex flex-wrap justify-between items-center'>
       <p className='text-sm text-muted-foreground'> &copy; {new Date().getFullYear()} Yahya. All right reserved.</p>

       <a href='#hero' className='p-2 rounded-full bg-primary/10 hover:bg-primary/20 transition-colors text-primary '>
        <ArrowUp size={20}/>
        </a>
            <p className='text-sm text-muted-foreground'>Made with ❤️ by 
                <a href=''>
                {" "}
                Syed Muhammad Yahya
                </a>
            </p>
        
    </footer>
    
  )
}

export default Footer
